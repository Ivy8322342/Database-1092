...
//  private JButton ReservationBtn;
  ...
  public res_b ()throws SQLException
  {
 //  ReservationBtn();
 //  CancelFinishPanel();
 
  }
  public Integer CheckFinishOrNot(String prebookID) throws SQLException {
		Connection conn = null;
		ResultSet result = null;

		try {
			conn = connection_sql.getConnection();
			PreparedStatement stat = conn.prepareStatement("SELECT Finish_Or_Not FROM APPOINTMENTPICKUP WHERE Prebook_ID = ?");
			stat.setString(1, prebookID);
			result = stat.executeQuery();
			while (result.next()) {
				FinishOrNot = result.getInt("Finish_Or_Not");
		...
		System.out.println(FinishOrNot);
		return FinishOrNot;
	}
