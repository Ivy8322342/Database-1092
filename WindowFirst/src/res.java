...
//	rePosition();
	...
	public static void main(String[] args) throws SQLException {
		EventQueue.invokeLater(new Runnable() {
			Prebuy pp=new Prebuy("ivy2046");
			public void run() {
				try {
					res window = new res(pp);
	 ...
	 * @throws SQLException
	 */
	public res(Prebuy prebuy) throws SQLException {
		this.prebuy = prebuy;
		initialize();
...
//				pwindow.frame.setVisible(true);
		...
		// add to Spanel
		addGood("義美全糖豆漿", "政治", "/img/義美全糖豆漿.jpg", 4);
		addGood("義", "政治", "/img/義美全糖豆漿.jpg", 5);
		addGood("義美全糖豆漿", "政治", "/img/義美全糖豆漿.jpg", 5);
		System.out.println("curSize " + curSize + " maxP: " + maxPage + " curPage: " + curPage);

		showPage();

//我覺得昨天換頁不成功應該是忘記加在initial，但我不知道要放在哪個frame，所以下面的frame只是示意		
		
		JButton lastButton = new JButton("上一頁");
		lastButton.setBounds(83, 405, 98, 25);
		reserveframe.getContentPane().add(lastButton);
		...
		reserveframe.getContentPane().add(nextButton);
